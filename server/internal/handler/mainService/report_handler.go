...
import (
	...
	mainService "server/internal/service/mainService"
	mainValidation "server/internal/validation/mainService"
	"server/pkg/logger"
...
func CreateReportHandler(c *fiber.Ctx) error {
	...
	req := mainDTO.CreateReportRequest{
	...
	db := database.GetDB()
	if err := mainValidation.Validate.Struct(req); err != nil {
		errors := mainValidation.FormatCreateReportValidationErrors(err)
	...
	result, err := mainService.CreateReport(db, userID, req)
	if err != nil {
