...
interface ReportInteractionBarProps {
    reportId?: number;
    reactionStats: ReactionStatsType;
        ...
        <div className="border-t border-gray-100 pt-3 mt-4">
            <div className="flex items-center justify-between text-sm text-gray-500 mb-3">
                <div className="flex items-center space-x-4">
                    {(reactionStats.likes > 0 || reactionStats.dislikes > 0) && (
                        <div className="flex items-center space-x-1">
                            {reactionStats.likes > 0 && (
                                <div className="flex items-center space-x-1">
                                    <div className="w-4 h-4 bg-red-500 rounded-full flex items-center justify-center">
                                        <FaHeart className="w-2 h-2 text-white" />
                                    </div>
                                    <span>{reactionStats.likes}</span>
                                </div>
                            )}
                            {reactionStats.dislikes > 0 && (
                                <div className="flex items-center space-x-1">
                                    <div className="w-4 h-4 bg-gray-500 rounded-full flex items-center justify-center">
                                        <FaThumbsDown className="w-2 h-2 text-white" />
                                    </div>
                                    <span>{reactionStats.dislikes}</span>
                                </div>
                            )}
                        </div>
                    )}
